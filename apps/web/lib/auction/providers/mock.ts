...
export interface MockProviderOptions {
  ...
  calculate?: Parameters<typeof mapLotSummary>[1] extends infer O
    ? O extends { calculate?: infer C }
      ? C
      : never
    : never;
}
...
export class MockAuctionProvider implements AuctionProvider {
  private readonly brands: RawBrand[];
  private readonly lots: RawLot[];
  private readonly calculate?: MockProviderOptions["calculate"];
  ...
  constructor(opts: MockProviderOptions = {}) {
    if (opts.brands && opts.lots) {
      ...
      this.brands = opts.brands ?? parsed.brands;
      this.lots = opts.lots ?? parsed.lots;
    }
